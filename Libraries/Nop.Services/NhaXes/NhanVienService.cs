...
namespace Nop.Services.NhaXes
{
    public class NhanVienService : INhanVienService
    {
        ...
        public virtual List<NhanVien> GetAllNhanVien(int NhaXeId, int VanPhongId, string TenNhanVien = "", ENKieuNhanVien[] kieunhanviens = null)
        {
            var query = _tableRepository.Table;
            query = query.Where(m =>m.NhaXeID==NhaXeId && !m.isDelete);
            if (VanPhongId > 0)
                query = query.Where(m => m.VanPhongID == VanPhongId);
            if (!string.IsNullOrEmpty(TenNhanVien))
            {
            ...
            if (kieunhanviens != null && kieunhanviens.Length > 0)
            {
                var kieunvids = kieunhanviens.Select(c => (int)c).ToArray();
                query = query.Where(m => kieunvids.Contains(m.KieuNhanVienID));
            }
            return query.ToList();
        }
        public virtual List<NhanVien> GetAllByVanPhongId(int VanPhongId, string TenNhanVien = "", ENKieuNhanVien[] kieunhanviens = null)
        {
